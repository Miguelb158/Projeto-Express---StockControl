...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($cargo) || empty($login) || empty($senha)) {
        ...
        $verifica = $conn->prepare("SELECT * FROM usuarios WHERE login = ?");
        ...
        if ($resultado->num_rows > 0) {
            ...
            $sql = $conn ->prepare("INSERT INTO usuarios (cargo, login, senha) VALUES (?, ?, ?)");
...
<head>
  ...
  <title>Cadastrar - Móveis Armazenamento</title>
